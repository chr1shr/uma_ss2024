...
void FixWallGran::post_force(int vflag)
{
  ...
  for (int i = 0; i < nlocal; i++) {
	...
	if (delr > radius[i]) dz = cylradius;
      ...
      } else if (wallstyle == ZFUNNEL) {
	delxy = sqrt(x[i][0]*x[i][0] + x[i][1]*x[i][1]);
	delr = delxy-wc0;
	if (x[i][2]>hi) {
	  if (delr+wc1*(x[i][2]-hi)<0) {
	    dz=x[i][2]-hi;
	  } else {
	    dz=((x[i][2]-hi)-wc1*delr)*wc3;
	    delr=(wc1*((hi-x[i][2])+wc1*delr))*wc3;
	  }
	} else if (x[i][2]<lo) {
	  if (delr+wc2*(lo-x[i][2])<0) {
	    dz=x[i][2]-lo;
	  } else {
	    dz=((x[i][2]-lo)+wc2*delr)*wc4;
	    delr=(wc2*((x[i][2]-lo)+wc2*delr))*wc4;
	  }
	} else {
	  dz=0;
	}
	dx = delr/delxy * x[i][0];
	dy = delr/delxy * x[i][1];
      } else if (wallstyle == ZSLIT) {
	dx = (x[i][0]>0?x[i][0]:-x[i][0])-wc0;
	if (x[i][2]>hi) {
	  if (dx+wc1*(x[i][2]-hi)<0) {
	    dz=x[i][2]-hi;
	  } else {
	    dz=((x[i][2]-hi)-wc1*dx)*wc3;
	    dx=(wc1*((hi-x[i][2])+wc1*dx))*wc3;
	  }
	} else if (x[i][2]<lo) {
	  if (dx+wc2*(lo-x[i][2])<0) {
	    dz=x[i][2]-lo;
	  } else {
	    dz=((x[i][2]-lo)+wc2*dx)*wc4;
	    dx=(wc2*((x[i][2]-lo)+wc2*dx))*wc4;
	  }
	} else dz=0;
	if(x[i][0]<0) dx=-dx;
	dy = 0;
			...
			double radius, double mass, int i)
{
  ...
  if(fw!=NULL) fw->store_wear_wall(i,dx,dy,dz,ccel*r*vrel*update->dt);
  if(fe!=NULL) fe->record_force(i,fx,fy,fz,-2*dx,-2*dy,-2*dz);
}
				...
				double radius, double mass, double *shear, int i)
{
  ...
  if (fs > fn) {
    if (shrmag != 0.0) {
      ...
      double ds0((fn/fs) * (shear[0] + sf*vtr1) - sf*vtr1),
             ds1((fn/fs) * (shear[1] + sf*vtr2) - sf*vtr2),
	     ds2((fn/fs) * (shear[2] + sf*vtr3) - sf*vtr3);
      if(fw!=NULL) {
        double dshear=sqrt((ds0-shear[0])*(ds0-shear[0])
	    +(ds1-shear[1])*(ds1-shear[1])+(ds2-shear[2])*(ds2-shear[2]));
	double wr=dshear*ccel*r;
	fw->store_wear_wall(i,dx,dy,dz,wr);
      }
      shear[0]=ds0;shear[1]=ds1;shear[2]=ds2;
      fs1 *= fn/fs ;
      fs2 *= fn/fs;
      fs3 *= fn/fs;
  ...
  // Store wear information if a wear fix is being used
  if(fe!=NULL) fe->record_force(i,fx,fy,fz,-2*dx,-2*dy,-2*dz);
}
